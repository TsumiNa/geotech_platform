...
def _load_csv(path):
    with open(path, "r", encoding="utf-8-sig") as fh:
        return [
            ...
            if not r.get("term_id", r.get("map_id", "x")).startswith("#")
        ]
...
class InferenceEngine:
    def __init__(self):
        self.L1 = sorted(
            _load_csv(os.path.join(KB, "L1_geology_terms.csv")),
            key=lambda r: int(r["priority"]),
        )
        self.L2 = sorted(
            _load_csv(os.path.join(KB, "L2_geology_to_geotech.csv")),
            key=lambda r: int(r["priority"]),
        )
        self.L3 = _load_yaml(os.path.join(KB, "L3_property_ranges.yaml"))[
            "ground_types"
        ]
        self.L4 = _load_yaml(os.path.join(KB, "L4_recommendations.yaml"))[
            "ground_types"
        ]
        self.sources = _load_yaml(os.path.join(KB, "sources.yaml"))["sources"]
        self.ontology = _load_yaml(os.path.join(REF, "geotech_ontology.yaml"))[
            "classes"
        ]
        self.risk = _load_yaml(os.path.join(REF, "engineering_risk_dictionary.yaml"))[
            "flags"
        ]
        self.rules = _load_yaml(os.path.join(REF, "geology_to_geotech_rules.yaml"))[
            "rules"
        ]
    ...
    def step_geology(self, unit):
        ...
        for row in self.L1:
            kws = [
                k.strip()
                for k in (row.get("match_keywords_ja") or "").split(";")
                if k.strip()
            ]
        ...
        if matched is None:
            return {
                "step": "L1_geology",
                "title_en": "Geology identification",
                "title_ja": "地質の同定",
                "inputs": {
                    "raw_unit_name_ja": name,
                    "raw_lithology": litho,
                    "raw_age": age,
                },
                "knowledge_ref": None,
        ...
        evidence = [
            f"Matched '{matched_kw}' in {where} -> {matched['term_id']} "
    ...
    def step_translate(self, geol_out):
        ...
        if matched is None:
            return {
                ...
                "title_en": "Geotechnical translation",
                "title_ja": "地盤工学への翻訳",
                "inputs": {
                    "geological_material": gm,
                    "grain_or_rock": gor,
                    "consolidation": cons,
                },
                "knowledge_ref": None,
                "outputs": {
                    "engineering_ground_type": "mixed_or_uncertain_geology",
    ...
    def step_properties(self, ground_type):
        ...
        return {
            ...
            "evidence": [
                "Indicative screening bands; NOT design values (design_use: false)."
            ],
    ...
    def step_recommendations(self, ground_type):
        ...
        return {
            ...
            "evidence": [
                "Conditional screening considerations ('may be relevant -> investigate'); not design conclusions."
            ],
            "sources": [],
            "confidence": "medium",
        }
    ...
    def infer(self, unit, asset_refs=None):
        ...
        needs_review = (
            overall_conf in ("low", "very_low")
            or gt == "mixed_or_uncertain_geology"
        ...
        return {
            ...
            "engineering_material_family": s2["outputs"].get(
...
if __name__ == "__main__":
    ...
    tests = [
        {
            "raw_unit_name_ja": "小山田層",
            "raw_lithology": "礫，砂及びシルト",
            "raw_age": "後期鮮新世-前期更新世",
            "source_region": "08062",
        },
        {
            "raw_unit_name_ja": "立川ローム層",
            "raw_lithology": "火山灰",
            "raw_age": "後期更新世",
            "source_region": "08063",
        },
        {
            "raw_unit_name_ja": "沖積層",
            "raw_lithology": "泥・砂及び礫",
            "raw_age": "完新世",
            "source_region": "08074",
        },
        {
            "raw_unit_name_ja": "埋立土",
            "raw_lithology": "砂及び泥",
            "raw_age": "完新世",
            "source_region": "08074",
        },
        {
            "raw_unit_name_ja": "柿生層",
            "raw_lithology": "泥岩",
            "raw_age": "前期更新世",
            "source_region": "08063",
        },
    ...
    for t in tests:
        ...
        for s in r["chain"]:
            print(
                f"  [{s['step']}] {s['title_en']} | conf={s['confidence']} | {s['knowledge_ref']}"
            )
