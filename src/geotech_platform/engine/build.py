...
Raw data is only READ (referenced from geology_reports/ via config).
"""
...
def build():
    ...
    B.write(regions)
    log(
        f"Boundaries: derived nominal quadrangles for {len(regions)} sheets (reviewable)."
    )
    ...
    for m in MC.MAPS:
        ...
        for i, (sh, rec) in enumerate(zip(shapes, recs)):
            ...
            if key not in unit_cards:
                asset_ids = [a["asset_id"] for a in region_assets.get(m["region"], [])][
                    :12
                ]
            ...
            feat = {
                ...
                **norm,
                "source_attribute_json": json.dumps(
                    {k: v for k, v in rec.items() if k != "_deleted"},
                    ensure_ascii=False,
                ),
            ...
            n += 1
        log(
            f"{m['region']} {m['map_name_en']}: {n} polygons normalized + inferred (CRS {crs})."
        )
    ...
    feat_cols = [
        ...
        "raw_lithology",
        "raw_age",
        "engineering_material_family",
        "engineering_ground_type",
        "geotech_term_ja",
        "geotech_term_en",
    ...
    IO.write_csv(
        os.path.join(config.PROC, "geology_features.csv"),
        [{k: f.get(k) for k in feat_cols[:12]} for f in features],
        feat_cols[:12],
    )
    IO.write_csv(
        os.path.join(config.PROC, "geotech_features.csv"),
        [
            {k: f.get(k) for k in feat_cols}
            | {"geotechnical_risk_flags": f["geotechnical_risk_flags"]}
            for f in features
        ],
    )
    IO.write_json(os.path.join(config.PROC, "inference_chains.json"), cards)
    IO.write_json(
        os.path.join(config.PROC, "geology_unit_cards.json"),
        [_card_summary(c) for c in cards],
    )
    IO.write_csv(
        os.path.join(config.PROC, "geology_unit_cards.csv"),
    ...
    log(
        f"Wrote processed CSV/JSON; {len(manual)} unit cards flagged for manual review."
    )
    ...
    write_gpkg(
        os.path.join(config.DATABASE, "tokyo_geotech_v2.gpkg"),
        {
            "geology_features": {"rows": norm_rows},
            "geotech_features": {"rows": geotech_rows},
        },
        srs_id=4612,
    )
    IO.write_sqlite(
        os.path.join(config.DATABASE, "tokyo_geotech_v2.sqlite"),
        {
            "geotech_features": [
                {
                    k: (
    ...
    pa = pdf_assets.build()
    log(
        f"PDF assets: extracted {pa['images']} figures, {pa.get('previews_png', 0)} previews; "
        f"see outputs/pdf_assets/. (08063 scanned/JBIG2 & 08074_D non-standard JPX flagged.)"
    )

    summary = {
        "features": len(features),
        "unit_cards": len(cards),
        "manual_review": len(manual),
        "pdf_figures": pa["images"],
    }
...
def _card_flat(c):
    return {
        "geology_unit_card_id": c["geology_unit_card_id"],
        "source_region": c["source_region"],
        "raw_unit_code": c["raw_unit_code"],
        "raw_unit_name_ja": c["raw_unit_name_ja"],
        "raw_unit_name_en": c["raw_unit_name_en"],
        "raw_lithology": c["raw_lithology"],
        "raw_age": c["raw_age"],
        "engineering_ground_type": c["engineering_ground_type"],
        "geotech_term_ja": c["geotech_term_ja"],
        "overall_confidence": c["overall_confidence"],
        "risk_flags": ";".join(c["risk_flags"]),
        "needs_manual_review": c["needs_manual_review"],
        "geology_chain": " -> ".join(
            s["knowledge_ref"] or s["step"] for s in c["chain"]
        ),
    }
