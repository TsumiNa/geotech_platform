...
DISCLAIMER_EN = (
    "Screening-level only: plausible ground-condition hypotheses, risk "
    "flags, uncertainty and recommended investigation. NOT design values."
)
DISCLAIMER_JA = (
    "机上スクリーニング用：想定地盤条件・リスク・不確実性・推奨調査のみ。"
    "設計値ではありません。"
)
...
class V2Index:
    ...
    def _load(self):
        ...
        attrs = {}
        with open(
            os.path.join(config.PROC, "geotech_features.csv"), encoding="utf-8-sig"
        ) as fh:
    ...
    def containing(self, lon, lat):
        for f in self.features:
            if G.point_in_bbox(lon, lat, f["bbox"]) and G.point_in_polygon(
                lon, lat, f["parts"]
            ):
                return f
        return None
...
def query_point(lat, lon):
    ...
    if not f:
        out["message_en"] = (
            ("Inside sheet %s but no polygon at point" % region_hits)
            if region_hits
