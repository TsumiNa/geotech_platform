...
from geotech_platform.engine import boundaries as B
from geotech_platform.engine import config, i18n
from geotech_platform.engine import query as Q
...
TT = {
    "title": {
        "en": "Geotechnical Geology Platform v2",
        "ja": "地質→地盤工学プラットフォーム v2",
    },
    "subtitle": {
        "en": "Geology → geotech inference with full drill-down to source evidence",
    ...
    "drilldown": {
        "en": "How this conclusion was reached (drill down)",
    ...
    "src_evidence": {
        "en": "Source evidence (raw attributes + report figures)",
        "ja": "根拠資料（生属性＋報告書の図表）",
    },
    "raw_attrs": {"en": "Raw map attributes", "ja": "地質図の生属性"},
    "related_figs": {
        "en": "Related report figures (same sheet)",
        "ja": "関連する報告書図表（同一図幅）",
    },
    "no_design": {
        "en": "Indicative screening bands — NOT design values.",
        "ja": "想定スクリーニング値であり、設計値ではありません。",
    },
    "outside": {
        "en": "Outside mapped extent / no polygon at this point.",
        "ja": "地質図範囲外、またはこの地点にポリゴンがありません。",
    },
    "lat": {"en": "Latitude", "ja": "緯度"},
    "lon": {"en": "Longitude", "ja": "経度"},
    "run": {"en": "Query", "ja": "照会"},
    "dist_boundary": {"en": "Distance to geology boundary", "ja": "地質境界までの距離"},
    "pick_unit": {"en": "Pick a geology unit", "ja": "地質単元を選択"},
    "filter_label": {
        "en": "Filter figures by content label",
        "ja": "内容ラベルで図を絞り込み",
    },
}
...
def load_chains():
    return json.load(
        open(os.path.join(PROC, "inference_chains.json"), encoding="utf-8")
    )
...
def render_step(step, idx):
    ...
    badge = {"high": "🟢", "medium": "🟡", "low": "🟠", "very_low": "🔴"}.get(
        step["confidence"], "⚪"
    )
    with st.expander(f"{idx}. {title}  {badge} {CONF(step['confidence'])}"):
        c1, c2 = st.columns(2)
        c1.markdown(f"**{T('inputs')}**")
        c1.json(step.get("inputs", {}))
        c2.markdown(f"**{T('outputs')}**")
        c2.json(step.get("outputs", {}))
        ...
        if step.get("sources"):
            ...
            for s in step["sources"]:
                if s.get("url"):
                    st.write(
                        f"• [{s['id']}] [{s['title']}]({s['url']}) — *{s.get('tier', '')}*"
                    )
                else:
...
def render_chain(card, region):
    ...
    if flags:
        st.markdown(
            f"**{T('risk')}:** "
            + ("、".join(flags) if LANG == "ja" else ", ".join(flags))
        )
    ...
    order = {
        "L4_considerations": 0,
        "L3_indicative_properties": 1,
        "L2_geotech_translation": 2,
        "L1_geology": 3,
    }
...
def _show_region_figures(region, limit=6, label_filter=None):
    ...
    if label_filter:
        a = a[
            a.content_label.astype(str).str.contains(label_filter, case=False, na=False)
        ]
    a = a.head(limit)
    if a.empty:
        st.caption(
            "— no extracted figures for this sheet (08063/08074 monographs flagged for OCR)."
        )
        return
    ...
    for i, (_, r) in enumerate(a.iterrows()):
        ...
        if os.path.exists(path):
            cols[i % 3].image(
                path,
                caption=f"{r['asset_id']} (p{r.get('page_estimate', '?')}): {cap}",
                width="stretch",
            )
...
with tabs[0]:
    st.markdown(
        ...
        if LANG == "en"
        else "**GSJ 5万分の1地質図 → L1 地質同定 → L2 地盤工学翻訳 → L3 想定物性 → "
        "L4 留意点 → 根拠資料へドリルダウン**"
    )
    ...
    for r in regs:
        ...
        rows.append(
            {
                "region": r["region"],
                "map": f"{r['map_name_en']} / {r['map_name_ja']}",
    ...
    st.caption(
        "Nominal = official 1:50k quadrangle frame (used for region assignment). "
        "Data = actual polygon extent. Review/edit reference/region_boundaries.yaml."
        if LANG == "en"
        else "Nominal＝正式な5万分の1図幅枠（図幅判定に使用）。Data＝実ポリゴン範囲。"
...
with tabs[1]:
    ...
    picked = st.multiselect(
        T("ground_type"), list(lab2code.keys()), default=list(lab2code.keys())
    )
...
with tabs[2]:
    ...
    if st.button(T("run")):
        ...
        if not r["found"]:
            ...
            raw = r["raw"]
            nm = (
                raw["raw_unit_name_ja"]
                if LANG == "ja"
                else (raw["raw_unit_name_en"] or raw["raw_unit_name_ja"])
            )
            st.info(
                f"**{nm}** ({raw['raw_unit_code']}) · {raw['raw_lithology']} · {raw['raw_age']}  \n"
...
with tabs[3]:
    ...
    chains = load_chains()
    labels = [
        f"{c['source_region']} · {c['raw_unit_name_ja']} ({c['raw_unit_code']}) → {GT(c['engineering_ground_type'])}"
        for c in chains
    ]
    unit_index = st.selectbox(
        T("pick_unit"), range(len(labels)), format_func=lambda i: labels[i]
    )
    render_chain(chains[unit_index], chains[unit_index]["source_region"])
...
with tabs[4]:
    ...
    if assets.empty:
        ...
        st.caption(
            f"{len(assets)} figures extracted (08062 monograph). "
            "08063 is scanned (JBIG2) and 08074_D uses non-standard JPX — flagged for OCR."
        )
        labels = ["(all)"] + sorted(
            x for x in assets.content_label.dropna().unique() if x
        )
        label_filter = st.selectbox(T("filter_label"), labels)
        a = (
            assets
            if label_filter == "(all)"
            else assets[assets.content_label == label_filter]
        )
        a = a[a.preview_png.notna() & (a.preview_png.astype(str) != "")]
        cols = st.columns(3)
        for i, (_, r) in enumerate(a.head(30).iterrows()):
            ...
            if os.path.exists(path):
                ...
                cols[i % 3].image(path, caption=cap, width="stretch")
...
with tabs[5]:
    ...
    if df.empty:
        st.success(
            "No units flagged." if LANG == "en" else "要レビューの単元はありません。"
        )
    else:
